...
/**
 * Funkcja sortuje listê samochodów po marce w kolejnoœci malej¹cej. W wyniku sortowania,
 * samochody zostan¹ ustawione w nowej kolejnoœci, zgodnie z kryterium sortowania. Funkcja
 * modyfikuje istniej¹c¹ listê i zwraca wskaŸnik do nowej g³owy listy (jeœli kolejnoœæ siê
 * zmieni³a) lub wskaŸnik do oryginalnej g³owy listy (jeœli kolejnoœæ nie zmieni³a siê).
 *
 * \param glowa - WskaŸnik do g³owy listy samochodów.
 * \return WskaŸnik do g³owy listy samochodów po sortowaniu.
 */
Auto* sortuj_po_marce_malejaco(Auto* glowa) 
{
    ...
    while (aktualna != NULL)   /// Pêtla dopóki nie osi¹gniêto koñca listy
    {
        ...
        if (posortowana == NULL || strcmp(aktualna->marka, posortowana->marka) > 0)   /// Warunek jeœli posortowana lista jest pusta lub aktualny samochód ma wiêksz¹ markê
        {
        ...
        else
        {
            ...
            while (bufor->next != NULL && strcmp(bufor->next->marka, aktualna->marka) > 0)   /// Pêtla dopóki nie osi¹gniêto koñca listy bufora lub marka kolejnego samochodu jest wiêksza
            {
