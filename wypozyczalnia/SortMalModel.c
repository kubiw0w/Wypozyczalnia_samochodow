...
/**
 * Funkcja sortuje listê samochodów po modelu w kolejnoœci malej¹cej. W wyniku sortowania,
...
Auto* sortuj_po_modelu_malejaco(Auto* glowa)
{
    ...
    while (aktualna != NULL)   /// Pêtla dopóki nie osi¹gniêto koñca listy
    {
        ...
        if (posortowana == NULL || strcmp(aktualna->model, posortowana->model) > 0)   /// Warunek jeœli posortowana lista jest pusta lub aktualny samochód ma wiêkszy model
        {
        ...
        else
        {
            ...
            while (bufor->next != NULL && strcmp(bufor->next->model, aktualna->model) > 0)   /// Pêtla dopóki nie osi¹gniêto koñca listy bufora lub model kolejnego samochodu jest wiêkszy
            {
