...
/**
 * Funkcja sortuje listê samochodów po numerze rejestracyjnym w kolejnoœci malej¹cej.
 ...
 */
Auto* sortuj_po_numerze_rejestracyjnym_malejaco(Auto* glowa)
{
    ...
    while (aktualna != NULL)   /// Pêtla dopóki nie osi¹gniêto koñca listy
    {
        ...
        if (posortowana == NULL || strcmp(aktualna->nr_rejestracyjny, posortowana->nr_rejestracyjny) > 0)   /// Warunek jeœli posortowana lista jest pusta lub aktualny samochód ma wiêkszy numer rejestracyjny
        {
        ...
        else
        {
            ...
            while (bufor->next != NULL && strcmp(bufor->next->nr_rejestracyjny, aktualna->nr_rejestracyjny) > 0)   /// Pêtla dopóki nie osi¹gniêto koñca listy bufora lub numer rejestracyjny kolejnego samochodu jest wiêkszy
            {
