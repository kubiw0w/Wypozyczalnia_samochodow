...
/**
 * Funkcja sortuje listê samochodów po wartoœci roku produkcji w kolejnoœci malej¹cej.
 ...
 */
Auto* sortuj_po_roku_produkcji_malejaco(Auto* glowa) 
{
    ...
    while (aktualna != NULL) /// Pêtla dopóki nie osi¹gniêto koñca listy
    {
        ...
        if (posortowana == NULL || aktualna->rok_produkcji > posortowana->rok_produkcji) /// Warunek jeœli posortowana lista jest pusta lub aktualny samochód ma wiêkszy rok produkcji
        {
        ...
        else
        {
            ...
            while (bufor->next != NULL && bufor->next->rok_produkcji > aktualna->rok_produkcji) /// Pêtla dopóki nie osi¹gniêto koñca listy bufora lub rok produkcji kolejnego samochodu jest wiêkszy
            {
