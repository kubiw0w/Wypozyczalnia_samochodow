...
/**
 * Funkcja sortuje listê samochodów po wartoœci przebiegu w kolejnoœci rosn¹cej.
 ...
 */
Auto* sortuj_po_przebiegu_rosnaco(Auto* glowa)
{
    ...
    while (aktualna != NULL) /// Pêtla dopóki nie osi¹gniêto koñca listy
    {
        ...
        if (posortowana == NULL || aktualna->przebieg < posortowana->przebieg) /// Warunek jeœli posortowana lista jest pusta lub aktualny samochód ma mniejszy przebieg
        {
        ...
        else
        {
            ...
            while (bufor->next != NULL && bufor->next->przebieg < aktualna->przebieg) /// Pêtla dopóki nie osi¹gniêto koñca listy bufora lub przebieg kolejnego samochodu jest mniejszy
            {
